...
main (int argc, char **argv)
{
  ...
  assert_msg (rc == SV_FAILED_TO_VERIFY_SIGNATURE, "expected failed to verify signature, got rc = %d\n", rc);
  ...
  rc = pseries_apply_update (&auth_data, &new_data, &new_data_size, &new_timestamp,
                             &verified_flag);
  assert_msg (rc == SV_AUTH_SIZE_INVALID, "expected auth size invalid, got rc = %d\n", rc);
  ...
  rc = pseries_apply_update (&auth_data, &new_data2, &new_data2_size, &new_timestamp2,
                             &verified_flag);
  assert_msg (rc == SV_TIMESTAMP_IN_PAST, "expected time in past, got rc = %d\n", rc);
